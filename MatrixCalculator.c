...
int main(){
   ...
   // SetConsoleOutputCP(1251);
    ...
    default: printf("Введите правильное значение!\n");
    }
    }
...
//------------------------------------------------------------
void determinant(int **matr,int rows,int cols){   //òîëüêî äëÿ 2íà2 3íà3 ïðîâåðîê íåòó
	int i,j,k,l,det;
    do{
...
void inputRowsCols(int *rows,int *cols){
    ...
    fflush(stdin);
    if(*rows<=0 || *cols<=0) {
	printf("Некоректно введение даные! Повторите попытку!");
	}
    }while(*rows<=0 || *cols<=0);
}
//---------------------------------------------------    
